...
def _get_rec_names(self):
    ...
    model = self.env["ir.model"].sudo().search([("model", "=", str(self._name))])
...
def _extend_name_results(self, domain, results, limit):
    ...
    if result_count < limit:
        ...
        recs = self.sudo().search(domain, limit=limit - result_count)
...
class Base(models.AbstractModel):
    ...
    def get_view(self, view_id=None, view_type="form",**options):
        ...
        _logger.info("Patching get_view on Base")
...
class IrModel(models.Model):
    ...
    name_search_domain = fields.Char("Name Search Domain")
    ...
    def _patch_method(cls, name, method):
        ...
        class_type = type(cls)
        origin = getattr(class_type, name)
        method.origin = origin
        # propagate decorators from origin to method, and apply api decorator
        wrapped = api.propagate(origin, method)
        wrapped.origin = origin
        setattr(class_type, name, wrapped)

    def _register_hook(self):
        for model in self.sudo().search(self.ids or []):
            Model = self.env.get(model.model)
            if Model is not None:
                self._patch_method(Model, "name_search", patch_name_search())
