...
class ResUsers(models.Model):
    ...
    def write(self, vals):
        if vals.get('password'):
            self.sudo()._check_password(vals['password'])
            vals['password_write_date'] = fields.Datetime.now()
    ...
    def _check_password_rules(self, password):
        ...
        if estimation["score"] < company_id.password_estimate:
            estimation_msg = estimation["feedback"].get("warning")
            if not estimation_msg and estimation["feedback"].get("suggestions"):
                estimation_msg = estimation["feedback"].get("suggestions")[0]
            raise PassError(estimation_msg)
